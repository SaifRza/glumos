...
class Verify extends BaseController
{
    ...
    public function index($hashmail=null)
    {
        $employers=new Employers();
        $seekers=new JobSeekers();
        $verify=$employers->verifyEmployer($hashmail);
        if($verify){
         $seekers=new JobSeekers();
        return view('front/activation');    
        }else{
         $verify=$seeker->verifyJobSeeker($hashmail);
          $seekers=new JobSeekers();
        return view('front/activation'); 
        }
    ...
    if($selected=='jobseeker'){
     ...
     $jobseekerExists=$seeker->isJobseekerValid($email,$password);
     if($jobseekerExists){
     ...
     $employerExists=$employer->isEmployerValid($email,$password);
     if($employerExists){
